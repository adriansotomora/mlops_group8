...
def main(cfg: DictConfig) -> None:
    ...
    try:
        ...
        raw_art = run.use_artifact(cfg.data_validation.input_artifact)
        ...
        validate_data(df, config_dict=config_dict)
        ...
        if val_report_full.is_file():
            ...
            with open(val_report_full) as f:
                report = json.load(f)
        ...
        if val_report_full and val_report_full.is_file() and wandb.run is not None:
            artifact = wandb.Artifact("validation_report", type="report")
            artifact.add_file(str(val_report_full))
            run.log_artifact(artifact, aliases=["latest"])
        sys.exit(1)
