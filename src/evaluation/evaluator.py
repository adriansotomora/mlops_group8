...
def round_metrics_dict(metrics_dict: Dict[str, Any], ndigits: int = 4) -> Dict[str, Any]:
    ...
    for k, v in metrics_dict.items():
        if isinstance(v, dict): # For nested dicts, like confusion matrix (not used here but good pattern)
            rounded[k] = {ik: (round(iv, ndigits) if isinstance(iv, (float, np.floating, int)) else iv) for ik, iv in v.items()}
...
def calculate_regression_metrics_from_predictions(
    ...
    if len(y_true) != len(y_pred):
        raise ValueError(f"Length of y_true ({len(y_true)}) and y_pred ({len(y_pred)}) must be the same.")
    if y_true.empty:
        logger.warning("y_true is empty. Returning NaN for all metrics.")
        return {"mse": np.nan, "rmse": np.nan, "mae": np.nan, "r2": np.nan, "adj_r2": np.nan, "mape": np.nan}
    ...
    if n - p - 1 > 0:
        ...
        logger.warning(f"Cannot calculate Adjusted R-squared: n ({n}) - p ({p}) - 1 is not positive.")
...
def evaluate_statsmodels_model(
    ...
    if not selected_features:
        logger.warning(f"No features provided for evaluation of '{split_label}' split. Returning NaN metrics.")
        # Return NaN for a standard set of metrics
        return {"mse": np.nan, "rmse": np.nan, "mae": np.nan, "r2": np.nan, "adj_r2": np.nan, "mape": np.nan}

    if X_eval.empty or y_eval.empty:
        logger.warning(f"X_eval or y_eval is empty for '{split_label}' split. Returning NaN metrics.")
        return {"mse": np.nan, "rmse": np.nan, "mae": np.nan, "r2": np.nan, "adj_r2": np.nan, "mape": np.nan}
    ...
    if len(common_index) != len(X_eval_selected) or len(common_index) != len(y_eval):
        logger.warning(f"Indices of X_eval_selected and y_eval for '{split_label}' split do not perfectly match. "
                       f"Using common intersection of size {len(common_index)} for evaluation.")
    ...
    if X_eval_aligned.empty or y_eval_aligned.empty:
        logger.warning(f"X_eval or y_eval became empty after alignment for '{split_label}' split. Returning NaN metrics.")
        return {"mse": np.nan, "rmse": np.nan, "mae": np.nan, "r2": np.nan, "adj_r2": np.nan, "mape": np.nan}
    ...
    try:
        ...
        y_pred_series = pd.Series(y_pred, index=y_eval_aligned.index) # Ensure y_pred is a Series with matching index
        ...
        logger.error(f"Error during prediction or metric calculation for '{split_label}' split: {e}")
        logger.debug(f"Model exog_names: {model.model.exog_names if hasattr(model, 'model') else 'N/A'}")
        ...
        raw_metrics = {"mse": np.nan, "rmse": np.nan, "mae": np.nan, "r2": np.nan, "adj_r2": np.nan, "mape": np.nan}

    # Log rounded metrics
    rounded_metrics = round_metrics_dict(raw_metrics)
    log_msg_split = f" for '{split_label}' split" if split_label else ""
    logger.info(f"Regression Evaluation Metrics{log_msg_split}: {rounded_metrics}")
