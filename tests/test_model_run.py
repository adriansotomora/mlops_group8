"""
Tests for src/model/run.py module - Hydra script with WandB integration
"""
import pytest
...
class TestModelRun:
    """Test cases for model run module."""
    ...
    @patch('src.model.run.datetime')
    def test_main_success(self, mock_datetime, mock_main_modeling, mock_wandb, mock_config):
        """Test successful execution of main function."""
        from src.model.run import main
        ...
        mock_wandb.init.assert_called_once_with(
            ...
            entity='test_entity',
            job_type='model',
            name='model_20250101_120000',
            config=dict(mock_config),
            tags=['model']
        )
        
        # Verify main_modeling was called
        mock_main_modeling.assert_called_once_with(
            config_path=str(PROJECT_ROOT / "config.yaml")
        )
        
        # Verify logging
        mock_wandb.log.assert_called_with({"model_status": "completed"})
        mock_wandb.finish.assert_called_once()

    @patch('src.model.run.wandb')
    @patch('src.model.run.main_modeling')
    @patch('src.model.run.datetime')
    @patch('src.model.run.logger')
    def test_main_exception_handling(self, mock_logger, mock_datetime, mock_main_modeling, mock_wandb, mock_config):
        ...
        # Make main_modeling raise an exception
        test_exception = Exception("Test error")
        mock_main_modeling.side_effect = test_exception
        ...
        mock_logger.exception.assert_called_once_with("Failed during model step")
    ...
    @patch('src.model.run.main_modeling')
    @patch('src.model.run.datetime')
    def test_main_wandb_init_failure(self, mock_datetime, mock_main_modeling, mock_wandb, mock_config):
        ...
        # main_modeling should not be called if WandB init fails
        mock_main_modeling.assert_not_called()

    @patch('src.model.run.wandb')
    @patch('src.model.run.main_modeling')
    @patch('src.model.run.datetime')
    def test_main_no_wandb_run(self, mock_datetime, mock_main_modeling, mock_wandb, mock_config):
    ...
    @patch('src.model.run.main_modeling')
    @patch('src.model.run.datetime')
    def test_main_wandb_finish_called_on_success(self, mock_datetime, mock_main_modeling, mock_wandb, mock_config):
    ...
    @patch('src.model.run.main_modeling')
    @patch('src.model.run.datetime')
    def test_main_wandb_no_run_available(self, mock_datetime, mock_main_modeling, mock_wandb, mock_config):
    ...
    def test_project_root_path(self):
        ...
        from src.model.run import PROJECT_ROOT
    ...
    def test_src_root_path(self):
        ...
        from src.model.run import SRC_ROOT
        
        assert SRC_ROOT.is_dir()
        assert SRC_ROOT.name == "src"

    @patch('src.model.run.main')
